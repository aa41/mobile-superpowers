...
SCRIPT = ROOT / "scripts" / "mobile_flutter_web_verify.py"


def load_module():
    spec = importlib.util.spec_from_file_location("mobile_flutter_web_verify", SCRIPT)
...
class MobileFlutterWebVerifyTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_dry_run_returns_build_serve_screenshot_compare_report_plan(self) -> None:
        module = load_module()

        result = module.verify_flutter_web(
        ...
        self.assertIn(["flutter", "build", "web"], result["commands"])
    ...
    def test_execute_with_fake_adapters_runs_pipeline(self) -> None:
        ...
        def fake_runner(command, *, cwd=None, timeout=None):
            ...
            build_dir = self.project_dir / "build" / "web"
            build_dir.mkdir(parents=True, exist_ok=True)
            (build_dir / "index.html").write_text("<!doctype html><title>Flutter</title>", encoding="utf-8")
            return subprocess.CompletedProcess(command, 0, stdout="", stderr="")

        def fake_capture(**kwargs):
            Path(kwargs["screenshot_path"]).write_bytes(b"platform")
        ...
        self.assertTrue(any("flutter build web" in call for call in calls))
